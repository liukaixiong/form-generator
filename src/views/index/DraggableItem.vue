...
import elTabsMethod from "@/components/render/slots/el-tabs.js";
...
const layouts = {
  colFormItem(h, element, index, parent) {
    ...
    let parentGroupValue = config["parentGroupValue"];
  ...
  draggableFormItem(h, element, index, parent) {
    const config = element.__config__;
    const { activeItem } = this.$listeners;
    const className =
      this.activeId === element.__config__.formId
        ...
        : "drawing-row-item";
    let child = renderChildren.apply(this, arguments);
    let childNode = elTabsMethod.options(h, element, config.renderKey, child);
    ...
    return (
      <el-col span={element.__config__.span}>
        ...
        >
          <draggable
            element={element.__config__.tag}
            list={element.__config__.children}
            animation={340}
            group="componentsGroup"
            component-data={{
            ...
            class="drag-wrapper"
            onadd={event => {
              console.log("拖动组件加入的时候触发 : ", element);
              element.__config__.children.forEach(function(obj, index) {
                if (!obj.__config__["parentGroupValue"]) {
                  obj.__config__["parentGroupValue"] = element.__config__.defaultValue + "";
                }
              });
            }}
          >
...
function addDataObj(evt, data) {
  console.log("add Data obj ", evt, data);
}
